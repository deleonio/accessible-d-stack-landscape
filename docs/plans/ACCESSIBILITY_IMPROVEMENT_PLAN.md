...
- Einbindung in `CategoryGrid.tsx`: bei Änderung der gefilterten Anzahl Text wie „142 Einträge gefunden" (i18n-fähig, debounced 300 ms).
...
- SVG mit `role="img"` + `aria-labelledby` (Titel + Beschreibung).
- Tastaturnavigation zwischen Knoten (Pfeiltasten, Enter öffnet Detail).
- Äquivalente Tabellen-/Listendarstellung via Toggle „Als Liste anzeigen".
- Fokusindikator auf SVG-Knoten.
...
- Neue Route `/erklaerung-zur-barrierefreiheit` bzw. `/accessibility-statement`.
...
- CI-Gate: `lighthouse --only-categories=accessibility` (Score ≥ 95) als Pflicht-Check in GitHub Actions.
...
- [ ] Lighthouse-Accessibility-Score ≥ 95 auf allen Routen
...
1. **Zielsprachen der Erklärung:** Nur DE oder alle 8 i18n-Sprachen?
2. **Feedback-Kanal:** E-Mail-Adresse, GitHub-Issue-Template oder Kontaktformular?
3. **Schlichtungsstelle:** Bundesfachstelle Barrierefreiheit (Bund) oder Länder-Stelle?
4. **High-Contrast-Design:** Eigene Farbtokens oder Ableitung aus Dark-Mode-Werten?
5. **Nutzertest-Budget:** Ist ein Audit durch DIAS / BIK-BITV (extern, kostenpflichtig) vorgesehen?
